...
function display_graph(filter_week,filter_bor) {
	var next_step;
	$('#graph_result4').html('');
...
//	pager_graph++;
	//next_step = ;
	//alert(next_step);
	next_step = pager_graph + 1;
	pager_graph = next_step;
	
	console.log('coucou =>' + next_step);
	...
	previous_step = pager_graph - 1;
	pager_graph = previous_step;
	
	le.click(function () {
		pager_most_visited_station();
	});
	
	//console.log('pager =>' + pa + " ====" + p);
...
function pager_most_visited_station() {
	...
	display_graph(filter_week,filter_bor);
}
...
window.onload = function () {
		...
		paper.add(mapdata);
		var circles = [
			{"id":1,"x":84,"y":398,"radius":10,"station": "14 St-Union Square","text" : "The cool place \nto hang out for \nany high-schooler","nb":"20986 RIDERS"},
			{"id":2,"x":100,"y":324,"radius":10,"station": "9 St Columbus Circle","text" : "","nb":"17596 RIDERS"},
			{"id":3,"x":99,"y":498,"radius":10,"station": "DeKalb Av","text" : "","nb":"14272 RIDERS"}
		...
		$.each(circles, function(key, value){
			...
			var t = paper.text(value.x + 50, value.y- 85, value.text);
			t.attr({ "font-size": 13, "font-family": "BebasRegular" });
			t.hide();
